...
#+begin_src bash
./batch_telemetry_15.sh start  batch_1 "10M row import"
./batch_telemetry_15.sh sample batch_1
./batch_telemetry_15.sh end    batch_1
...
# Start telemetry
./batch_telemetry_15.sh --table "$TABLE_NAME" start "$BATCH_ID" "10M row insert"

# Run the batch insert job
# <your insert command here>

# End telemetry and generate report
./batch_telemetry_15.sh end "$BATCH_ID"
./batch_telemetry_15.sh report "$BATCH_ID" > "${BATCH_ID}_report.txt"
#+end_src
...
while true; do
  ./batch_telemetry_15.sh sample "$BATCH_ID"
  sleep 60
done
#+end_src
