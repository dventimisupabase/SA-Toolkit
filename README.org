...
Client-side batch telemetry for PostgreSQL 15, 16, or 17. Captures before/after snapshots of database statistics to diagnose checkpoint pressure, WAL volume, autovacuum interference, and I/O bottlenecks during batch workloads—without writing anything to the database.
...
#+begin_src bash
./batch-telemetry/batch_telemetry.sh start  batch_1 "10M row import"
./batch-telemetry/batch_telemetry.sh sample batch_1
./batch-telemetry/batch_telemetry.sh end    batch_1
...
# Start telemetry
./batch-telemetry/batch_telemetry.sh --table "$TABLE_NAME" start "$BATCH_ID" "10M row insert"

# Run the batch insert job
# <your insert command here>

# End telemetry and generate report
./batch-telemetry/batch_telemetry.sh end "$BATCH_ID"
./batch-telemetry/batch_telemetry.sh report "$BATCH_ID" > "${BATCH_ID}_report.txt"
#+end_src
...
while true; do
  ./batch-telemetry/batch_telemetry.sh sample "$BATCH_ID"
  sleep 60
done
#+end_src
